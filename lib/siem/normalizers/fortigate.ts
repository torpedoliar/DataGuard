...
export function normalizeFortigate(message: string) {
  ...
  if (/vpn/i.test(message) && /(success|accept|established|up\b)/i.test(message)) return { ...emptyNormalizedEvent(), category: "Firewall", normalizedType: "vpn_login_success", action: "login", outcome: "success", username: field(message, "user"), srcIp: field(message, "remip"), metadata: { vendor: "fortigate" } };
