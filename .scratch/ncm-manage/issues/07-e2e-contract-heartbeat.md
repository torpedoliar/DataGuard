...
**Status:** done (NCM `6923c73`; DG `6a192a2`; salinan brief sisi DG)

- [x] Test kontrak HTTP nyata NCM↔DG hijau: `app_v4/tests/e2e_server.py` (uvicorn live) ↔ DG `scripts/ncm-contract-probe.ts` — path, auth (X-API-Key + scopes), CRUD switch/credential, backup, baseline, drift→pending review, approve via `reviews:write`, rollback script. (HMAC ingest diuji di tiket 06: 22 test.)
- [x] Heartbeat: test-connection DG menulis `lastSeenAt` (`touchNcmLastSeen`); banner offline membacanya. (Worker penuh menyusul di tiket 09.)
