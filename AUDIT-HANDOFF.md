...
- [ ] **#02 · HIGH · misscode · auth-admin-routing-i18n** — Read-only server actions have no auth guard; missing session/activeSiteId silently drops the site filter and returns ALL sites' inventory
  - **File:** `actions/master-data.ts:126-158 (getDevices), 37-39 (getCategories); actions/rack-management.ts:22-39 (getRacks), 82-89 (getRackById); actions/brands.ts:60-62 (getBrands)`
  - **Bukti:** Verified pattern: getDevices does `const siteFilter = session?.activeSiteId ? eq(devices.siteId, session.activeSiteId) : undefined` then `.where(and(siteFilter, rackFilter))` — with no session or a null activeSiteId the filter is undefined and ALL devices across all sites are returned, including ipAddress, assetCode, photoPath. getBrands/getCategories have no auth check at all; getRacks has the same undefined-drops-filter bug; getRackById has no siteId condition and no callers. 'use server' actions are reachable as POST endpoints by any same-origin client, logged-out visitors included.
  - **Dampak:** An unauthenticated caller can invoke getDevices/getRacks and receive the full cross-site inventory (device names, IP addresses, asset codes, rack positions, photo paths) — breaking the per-site isolation the middleware and page guards otherwise enforce.
  - **Saran:** Guard every read action with requireActiveSiteAction() and return [] when unauthorized or activeSiteId is null — never fall through to an unfiltered query. Add eq(racks.siteId, auth.activeSiteId) to getRackById, and decide whether getBrands/getCategories are site-scoped or superadmin-gated.
