...
- [x] **#01 · CRITICAL · bug · device-groups** — PIC owner persistence is broken: users.responsible_for_groups is declared `text` in the Drizzle schema but the migration created `jsonb` — every owner save fails or corrupts the column
  - **File:** `db/schema.ts:102 (users.responsibleForGroups)`; `drizzle/0029_devices_pics.sql:4`; `actions/device-groups.ts:177-196`; `db/schema.test.ts`
  - **Bukti:** Fixed and code-level verified: `users.responsibleForGroups` now uses Drizzle `jsonb("responsible_for_groups").$type<string[]>()`, matching migration 0029's existing PostgreSQL `jsonb NOT NULL DEFAULT '[]'::jsonb` contract. Drizzle's JSONB mapper serializes owner arrays such as `["42","99"]` to the independent JSON value `["42","99"]`, and an empty owner list to `[]`, instead of node-postgres's PostgreSQL array literal (`{"42","99"}`) or object-like `{}`. The existing PIC assignment/detach callers continue passing JavaScript string arrays, and the focused regression test verifies the column SQL type, non-empty serialization, empty serialization, and read mapping; `npx vitest run db/schema.test.ts` passed 1/1 and targeted ESLint passed with 0 errors. No users type migration was added because migration 0029 already creates the correct JSONB column; the separate `devices.responsible_groups` migration/schema drift remains finding #09 scope. Live PostgreSQL save/re-read verification is unavailable because no local PostgreSQL server is running.
  - **Dampak:** PIC owner writes now use the database's actual JSONB contract, so non-empty assignments no longer reach PostgreSQL as invalid array literals and empty detachments persist as iterable JSON `[]`. The remaining verification is an authenticated save/reload against the deployed database; transactionality and site/owner validation remain separate findings.
  - **Saran:** Keep the JSONB schema mapping and `[]` JSON default, do not generate a type-change migration against the already-JSONB users column, and add the authenticated PostgreSQL save/re-read smoke test when the database environment is available. Handle the separate `devices.responsible_groups` drift under finding #09 rather than folding it into this fix.
...
- [x] **#05 · HIGH · incomplete · cross-cutting** — Deactivated users / password changes do not revoke existing JWT sessions (7-day expiry, no DB check on verify)
  - **File:** `lib/session-token.ts`; `lib/session-auth.ts`; `lib/session.ts`; `middleware.ts`; `actions/auth.ts`; `lib/session-token.test.ts`; `lib/session-auth.test.ts`; `actions/auth.test.ts`
  - **Bukti:** Fixed and code-level verified: JWT creation now includes a non-reversible HMAC-SHA-256 fingerprint derived from the current `users.passwordHash`; `decrypt()` rejects tokens without the required 64-character fingerprint, so legacy signed JWTs fail closed. `verifySession()` and the Node.js middleware both call `validateSessionPayload()`, which reads the current user row and requires `isActive === true`, an unchanged global role, and a fingerprint recomputed from the current password hash. Missing users, disabled users, role changes, password changes, malformed/legacy tokens, and database lookup errors all resolve to an unauthenticated session. Login creates the fingerprint and `switchSite` carries forward only the already-validated fingerprint. Regression coverage passed 17/17 targeted tests, including active, missing, inactive, role-changed, password-changed, DB-error, and legacy-token cases. `actions/auth.test.ts` now mocks `next/headers` so the existing login redirect assertions exercise the updated caller; the auth tests passed as part of the targeted run. `npm run build` passed and compiled the middleware as a Node.js Proxy. PostgreSQL-backed authenticated/runtime verification remains unavailable because no local PostgreSQL server is running; full-suite verification remains limited by 7 unrelated existing failures (437 passed, 7 failed), full lint has 14 unrelated existing errors, and `npx tsc --noEmit` has the known baseline 18 errors in `actions/auth.test.ts`, `lib/env.test.ts`, and `lib/siem/receiver.test.ts`.
  - **Dampak:** A valid seven-day JWT is no longer sufficient after account disablement, role change, password reset/change, legacy-token use, or inability to confirm current user state; server actions and protected-route middleware now fail closed and require re-authentication.
  - **Saran:** The current-user DB validation and password-hash fingerprint controls are implemented without a new schema migration, avoiding drift with the existing uncommitted migration work. Keep the fingerprint claim derived and non-reversible, retain the fail-closed behavior, and add authenticated integration tests against PostgreSQL when the runtime environment is available.
...
- [x] **#07 · HIGH · bug · cross-cutting** — deleteDevice forceDelete destroys checklist history then fails on incidents FK (RESTRICT) — no transaction, permanent data loss
  - **File:** `actions/master-data.ts:336-344; db/schema.ts:215,225-226`
  - **Bukti:** Fixed and code-level verified: the audit claims were re-verified against the current code and migrations first — `deleteDevice(id, reason?, forceDelete=false)` deleted `checklistItems` first (outside any transaction) and only then attempted `DELETE devices`, while `incidents.device_id`, `checklist_items.device_id`, `network_ports.device_id`, `network_ports.connected_to_device_id`, `syslog_sources.device_id`, `siem_findings.device_id`, and `siem_evidence_events.device_id` are all `REFERENCES devices(id) ON DELETE NO ACTION` (0000/0001/0005/0008), `device_pics.device_id` is `ON DELETE CASCADE` (0029); the modal's "Check Device Usage" called the destructive `deleteDevice(deviceId, reason || "Checking...", false)`. The fix now adds a read-only preflight `getDeviceDeletionUsage(id)` (returns per-reference counts incl. `network_ports.connected_to_device_id` linked ports and a 10-row checklist history preview; calls `requireActiveSiteAction()`, never mutates), rewires the modal so "Check Device Usage" calls only the preflight and replaces the "Force Delete historical data" checkbox with a message that related data cannot be deleted and the device must be decommissioned via `toggleDeviceStatus` (component name + props interface unchanged for the hosting admin page), and rewrites `deleteDevice(id, reason?)` (forceDelete parameter removed; only caller is the modal) to hard-delete ONLY devices with zero references inside `db.transaction`: the device is re-read with the active-site scope in the tx, all nine dependency counts are re-checked in the tx, only the device row is deleted (device_pics cascades via its FK), and the DB FK acts as the final race guard — NO cascade and no deletion of checklist/incidents/ports/syslog/SIEM history ever. Upload photo deletion, `revalidatePath`, and `logAudit` all run only AFTER the transaction commits. `actions/master-data.test.ts` (new) covers with a mocked transaction (lib/siem/evidence.test.ts pattern) + guard/query-action shape (actions/read-actions-auth.test.ts pattern): unauthorized/no-active-site → no query/mutation/filesystem delete; preflight never deletes anything even with zero checklist items; checklist+incident fixture → blocked with no DELETE checklist_items, no DELETE devices, no photo deletion; zero-dependency device → deleted inside the tx (site-scoped `[42,7]` predicates) with photo/audit/cache strictly after commit; ports+linked ports+syslog/SIEM families detected and blocked with counts surfaced; device_pics-only (cascade) is NOT a blocker; missing-device in tx; FK-race and generic transaction failure → no photo deletion, no audit, no history deletion. Validation: `npx vitest run actions/master-data.test.ts` 15/15 passed, `npx vitest run actions/` 44/44 passed (8/8 files), full suite 463 passed / 5 failed where all 5 are the pre-existing finding-#43 baseline (update-scripts x2, deploy-secrets x2, lib/incidents x1; no new failures), and `npx eslint actions/master-data.ts components/admin/delete-device-modal.tsx actions/master-data.test.ts` reported 0 errors (5 pre-existing-style unused-catch-var warnings, exit 0). `npm run build`, `npm run db:generate`, and `npm run db:migrate` were intentionally not run: build is reserved for the coordinator's single central build, db:generate is forbidden until finding #13, and no local PostgreSQL server exists on this machine, so live-DB FK/rollback verification was not possible.
...
- [x] **#09 · HIGH · misscode · database-schema-migrations** — devices.responsible_groups exists in the Drizzle schema but no migration ever creates it — and nothing reads/writes it; poisons db:generate and can crash fresh DBs
  ...
  - **Dampak:** Fresh/existing databases lack the column: the next `npm run db:generate` emits an unexpected ALTER TABLE devices ADD COLUMN (and db:push would alter the live DB), and any devices query selecting all columns fails on databases that never got the column ad hoc. New maintainers may also write PIC data into a column nothing reads. **Fixed and verified (commits d5cf024 + 2f15a42):** devices.responsibleGroups is now `jsonb` in schema.ts matching migration 0031_device_responsible_groups.sql (`ALTER TABLE "devices" ADD COLUMN IF NOT EXISTS "responsible_groups" jsonb NOT NULL DEFAULT '[]'::jsonb`, journal idx 31 registered) — fresh and existing DBs no longer crash devices queries, and db:generate stops proposing a phantom ALTER. The column is intentionally unused: PIC membership is stored in device_pics + users.responsible_for_groups (feature committed in 2f15a42).
...
- [x] **#14 · HIGH · misscode · ui-build** — deploy.sh/update.sh/update.ps1 melewatkan 4 service: siem-ai, siem-snapshot, incidents-overdue-notify, backup-scheduler
  - **File:** `deploy.sh L94, update.sh L134, update.ps1 L162; docker-compose.yml L126-197`
  - **Bukti:** Ketiga script mendefinisikan SIEM_WORKER_SERVICES=(syslog-receiver siem-parser siem-rules siem-alerts siem-retention) dan hanya `up -d` kelima itu. docker-compose.yml mendefinisikan 9 service stateless termasuk siem-ai, siem-snapshot, incidents-overdue-notify, backup-scheduler. restart: always/unless-stopped tidak meng-create ulang container saat image tag berubah. **Fixed and verified (commit 36c6666):** all three scripts now list all nine stateless services — deploy.sh:94, update.sh:134, update.ps1:162 include siem-ai, siem-snapshot, incidents-overdue-notify, backup-scheduler; `bash -n` (deploy.sh/update.sh) and PowerShell parser (update.ps1) checks pass.
...
- [x] **#15 · HIGH · bug · ui-build** — Split-brain env file: compose membaca .env, deploy.sh hanya menulis .env.production
  ...
  - **Dampak:** Deploy pertama di server bersih gagal atau memecah kredensial antara inisialisasi DB dan runtime app; deploy ulang membuat ulang password di file yang tidak dibaca siapa pun. Jalan hanya karena operator manual memelihara .env yang kebetulan cocok. **Fixed and verified (commit 8ec0a3a):** single env file `.env` — deploy.sh `ENV_FILE=".env"` writes/uses .env, update.sh defaults to .env, docker-compose.yml `env_file: .env` for all 11 services, header comments in deploy.sh and .env.example.production updated to say .env.
...
- [x] **#16 · MEDIUM · bug · telegram-messaging** — SIEM alert deep-link hardcodes ?severity=High even when the triggering finding is Critical or Medium
  - **File:** `lib/siem/alerts.ts:26; actions/siem-findings.ts:28`
  - **Bukti:** The alert template line 26 is 'Open: [Open in SIEM](${input.baseUrl}/admin/siem/findings?severity=High)' — severity is hardcoded. The findings list filters with exact match eq(siemFindings.severity, filters.severity). **Fixed and verified (commit 56032dd):** lib/siem/alerts.ts now interpolates `?severity=${input.severity}`; regression test asserts a Critical finding yields severity=Critical.
...
- [x] **#17 · MEDIUM · bug · telegram-messaging** — APP_URL override never takes effect: stored DB login host is prioritized despite the documented env override
  - **File:** `lib/notification-url.ts:32`
  - **Bukti:** 'return secureOrigin(stored || process.env.APP_URL || host || "localhost:3000")' — the comment states 'APP_URL env overrides when a public domain exists and login host is an internal IP', but stored (the last login host, typically the internal IP captured by actions/auth.ts:163-166) is checked before APP_URL, so APP_URL is unreachable whenever any stored value exists. **Fixed and verified (commit e6c6757):** priority is now `process.env.APP_URL || stored || host || "localhost:3000"` in lib/notification-url.ts, pinned by new lib/notification-url.test.ts (5 tests); lib/base-origin.test.ts updated.
...
- [x] **#18 · MEDIUM · bug · telegram-messaging** — secureOrigin forces https:// on plain-IP hosts, producing dead links on LAN/plain-HTTP deployments
  - **File:** `lib/base-origin.ts:7`
  - **Bukti:** 'return `${/localhost/i.test(base) ? "http" : "https"}://${base}`' — only localhost is treated as HTTP; any IP yields https (pinned by lib/base-origin.test.ts:8 expecting https://192.168.1.10:3000). The 0028 migration comment explicitly contemplates the 'login host is an internal IP' scenario, and docker-compose maps the app on port 3001 with no TLS. **Fixed and verified (commit c2cfe81):** secureOrigin now passes explicit http(s) schemes through untouched (https:// APP_URL keeps working), defaults external domains to https, and defaults localhost + plain IPv4/IPv6 hosts (LAN on-prem) to http; pinned tests updated (IP→http, [::1], explicit-scheme pass-through).
...
- [x] **#19 · MEDIUM · bug · telegram-messaging** — SIEM alert messages are sent completely unescaped: user content can garble formatting or break the deep link
  - **File:** `lib/siem/alerts.ts:17-30; lib/siem/redaction.ts`
  - **Bukti:** alertMessage interpolates raw title, summary, deviceName, sourceIp and passes them only through redactSensitiveText, which masks secrets but never escapes Markdown metacharacters. Legacy Markdown parses _, *, backtick, [; a stray '[' in a finding title earlier in the message can combine with the later '](' of the [Open in SIEM] link and corrupt link parsing. **Fixed and verified (commit e365e32):** entity fields (title, summary, siteName, deviceName, sourceIp, recommendedAction) now pass through escapeTelegramMarkdown (exported from lib/telegram.ts) via an esc() helper while the generated link and server-controlled fields stay raw; regression test covers a '[idle] _v2' title with the link intact. lib/siem + telegram suite 196/197 pass (1 pre-existing environmental receiver UDP-bind failure).
...
- [x] **#68 · LOW · incomplete · auth-admin-routing-i18n** — update.sh restarts only 5 of 9 worker services — stale code after deploy
  - **File:** `update.sh:134`
  ...
  - **Dampak:** After an update, siem-ai, siem-snapshot, incidents-overdue-notify and backup-scheduler keep running stale container code until manually restarted, so newly deployed logic silently diverges from the app. **Resolved by finding #14 (commit 36c6666):** update.sh:134 now lists all nine stateless services (including siem-ai, siem-snapshot, incidents-overdue-notify, backup-scheduler), and step 6 restarts every one of them after the rebuild. Duplicate of #14; no further change needed.
